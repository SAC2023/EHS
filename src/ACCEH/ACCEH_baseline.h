...
long c1=0;//declaration of variables for counting the k-v entry.
long c2=0;
...
constexpr size_t kSegmentSize = (1 << kSegmentBits) * 8 * 1;
...
int Segment<T>::Insert4split(T key, Value_t value, size_t loc) {
  for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
    ...
    //cout<<"counter in Insert4split = "<<c2<<endl;
  }
  ////cout<<"Counter in Insert4split = "<<c2<<endl;
  return -1;
}
...
template <class T>
CCEH<T>::CCEH(void) {
  std::cout << "Reintialize Up for ACCEH" << std::endl;
}
...
RETRY:
  ...
  for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
    auto slot = (y + (i*i)) % Segment<T>::kNumSlot;
    if constexpr (std::is_pointer_v<T>) {
