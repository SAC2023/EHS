...
int drc=0;
int spc=0;
long c1=0;//declaration of variable for counting k-v entry
long c2=0;
/*
...
int Segment::Insert(Key_t& key, Value_t value, size_t loc, size_t key_hash) {
  ...
  Key_t LOCK = INVALID;
  //quadratic probing
  for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {//c1++;
    auto slot = (loc + (i*i)) % kNumSlot;
    Key_t key_ = get_key(slot);
    if (CAS(&key_, &LOCK, SENTINEL)) {c1++;
...
//      _[slot].key = key;
    ...
    //cout<<"Counter in Insert = "<<c1<<endl;
  }
  lock = sema;
  while (!CAS(&sema, &lock, lock-1)) {
    lock = sema;
  }
  //cout<<"Count in Insert = "<<c1<<endl; //yene
  return ret;
}
...
void Segment::Insert4split(Key_t& key, Value_t value, size_t loc) {
	//quadratic probing
  for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {//c2++;
    auto slot = (loc + (i*i)) % kNumSlot;
    Key_t key_ = get_key(slot);
    if (key_ == INVALID) {c2++;
      pair_insert_pmem(slot,key,value);
   ...
   // cout<<"Counter in Insert4split = "<<c2<<endl; //yene
  }
  //cout<<"Count in Insert4split = "<<c2<<endl;
}
...
RETRY:
  ...
  if (ret == 1) {
    ...
    //cout<<"Split time cout "<<spc<<" = "<<t.GetSeconds()<<endl; //yeen
    if (s == nullptr) {
      goto RETRY;
    }
	...
	//t.Stop(); //yene
	//cout<<"Directory doubling round "<<drc<<" takes = "<<t.GetSeconds()<<endl; //yene
        delete d;
...
Value_t CCEH::Get(Key_t& key) {
 ...
 //quadratic probing
  for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
    auto slot = (y+ i*i) % Segment::kNumSlot;
