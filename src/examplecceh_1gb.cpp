...
// pool path and name
static const char *pool_name = "/mnt/pmem0/pmem_cceh_hash_1gb.data";
// pool size
...
int main() {
	...
	// Delete
	auto begindelete = chrono::high_resolution_clock::now();                                                                                           for (uint64_t i = 0; i < 1024; ++i) {                                                                                                                auto epoch_guard = Allocator::AquireEpochGuard();
		for (uint64_t j = 0; j < 1024; ++j) {
