...
DEFINE_string(index, "dash-ex",
              "the index to evaluate:dash-ex/dash-lh/cceh/acceh/level");
...
Hash<T> *InitializeIndex(int seg_num) {
  ...
  if (index_type == "dash-ex") {
    ...
    std::string index_pool_name = pool_name + "pmem_ex2.data";
  ...
  //////////////////////////////////////////ACCEH
  } else if (index_type == "acceh") {
	  std::cout << "Initialize ACCEH" << std::endl;
	  std::string index_pool_name = pool_name + "pmem_acceh.data";
	  if (FileExists(index_pool_name.c_str())) file_exist = true;
	  Allocator::Initialize(index_pool_name.c_str(), pool_size);
	  eh = reinterpret_cast<Hash<T> *>(Allocator::GetRoot(sizeof(acceh::CCEH<T>)));
	  if (!file_exist) {
		 new (eh) acceh::CCEH<T>(seg_num, Allocator::Get()->pm_pool_);
	  }else{
		 new (eh) acceh::CCEH<T>();
	  } 
  //////////////////////////////////////////////////
